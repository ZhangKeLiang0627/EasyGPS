...
void lv_port_disp_init(LGFX_ChappieCore *lcd)
{
    /*-------------------------
...
#if CHAPPIE_CORE_LVGL_USE_PSRAM
    ...
    // lv_color_t *buf_3_1 = (lv_color_t *)heap_caps_malloc(MY_DISP_HOR_RES * MY_DISP_VER_RES / 2 * sizeof(lv_color_t), MALLOC_CAP_DMA);
    // lv_color_t *buf_3_2 = (lv_color_t *)heap_caps_malloc(MY_DISP_HOR_RES * MY_DISP_VER_RES / 2 * sizeof(lv_color_t), MALLOC_CAP_DMA);

    /* If something wrong */
    if ((buf_3_1 == NULL) || (buf_3_2 == NULL))
    {
        // Serial.printf("[LVGL] malloc buffer from PSRAM error\r\n");
    ...
    // lv_disp_draw_buf_init(&draw_buf_dsc_3, buf_3_1, buf_3_2,
    //                       MY_DISP_VER_RES * LV_VER_RES_MAX / 2); /*Initialize the display buffer*/
...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    _lcd->startWrite();
    _lcd->writePixelsDMA((uint16_t *)&color_p->full, w * h, true);
    _lcd->endWrite();
    // _lcd->setSwapBytes(true);
    // _lcd->pushImageDMA(area->x1, area->y1, w, h, (uint16_t *)&color_p->full);
